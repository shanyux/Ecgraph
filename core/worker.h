...
void worker<update_type>::handle_graph_data(ecgraph::byte_t * buf, int len)
{
	...
	//写入文件
	if (m_graph_partition->is_open()) {//m_graph_partition是一个fstream,它返回一个布尔(bool)值，为真（true）代表文件已经被顺利打开，假( false )则相反。
...
void worker<update_type>::handle_update_data(ecgraph::byte_t * buf, int len)
{
	...
	if (!m_in_buffer->push(update_buf, update_len)) {//buffer
		LOG_TRIVIAL(error) << "worker(" << m_rank << ") m_in_buffer should be reset";
		exit(0);
	}
	/*if (m_rank == 2) {
...
void worker<update_type>::handle_hash_info_data(ecgraph::byte_t * buf, int len)
{
	...
	m_size = m_ring->worker_size() + 1;//全部的节点数
	m_ring->get_workers(m_machines);//获取工作节点编号vector
}
...
void worker<update_type>::handle_graph_info_data(ecgraph::byte_t * buf, int len)
{
	...
	//格式检查
	//检查该配置文件
	if (!m_partition_config->check()) {//该配置文件信息是否完整
...
inline void worker<update_type>::binay_partition_myself()
{
	...
	long long mid = std::stoll((*m_partition_config)["partition_mid_vid"]);//stoll将字符串转化成带符号整数
	...
	std::set<ecgraph::vertex_t> worker_set(machines.begin(), machines.end());//用machines初始化set
	...
	int pos = new_worker_partition_name.find_last_of('_');//find_last_of()查找最后一个出现的c。匹配的位置是返回值。如果没有匹配发生，则函数返回-1.
	...
	if (rename(m_partition_filename.c_str(), 
		...
		perror(m_partition_filename.c_str());//把一个描述性错误消息输出到标准错误 stderr
		exit(0);
	}
	...
	long long global_graph_vertices_num
		= std::stoll((*m_partition_config)["vertices"]);//int stoi( const std::string& str, size_t *pos = 0, int base = 10 );
...
inline void worker<update_type>::handle_data_sync(ecgraph::byte_t * buf, int len)
{
	...
	int index = 0;
	int *degree = m_algorithm->get_degree().data();// data返回当前容器的首元素指针，此时对于顺序容器可以直接用指针进行操作了。
	decltype(m_algorithm->get_result_value_type()) //更新值的类型
		*result = m_algorithm->get_result().data();
...
//a为图的平均度
template<typename update_type>
inline ecgraph::vertex_t worker<update_type>::get_graph_mid()//关键字inline 必须与函数定义体放在一起才能使函数成为内联
{
	//对排好序的图数据取中点
...
void worker<update_type>::recv()
{
	...
	bool go_on = true;
	ecgraph::byte_t *recv_buf = new ecgraph::byte_t[RECV_BUFFER_SIZE];//40M
	MPI_Status status;
