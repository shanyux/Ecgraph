...
public:
	...
	void start() {
		...
		//master_start();
		m_graphfile_buffer->start_write(m_graph_filename);//二进制文件
		go();
	}
...
private:
	...
	std::vector<ecgraph::vertex_t> m_machines;//vector
...
master::master(int argc,
	...
	m_ring = ring;
	m_graphfile_buffer = new ecgraph::buffer<ecgraph::edge_t>(GRAPH_DATA_BUFFER_SIZE);//申请数组

	m_size = m_ring->worker_size() + 1;//全部的工作节点数+1
	m_state_mutex = new std::mutex();
	
	m_ring->get_workers(m_machines);//得到所有的计算节点,顺时针排序的vector
	m_max_loop = std::stoi(argv[2]);//int stoi (const string&  str, size_t* idx = 0, int base = 10);
	m_current_loop = 0;
	m_graph_filename = argv[1];//二进制图数据
}
...
void master::sync_ring_info() {
	...
	std::string hash_info = m_ring->save();//获取哈希表json信息，返回string
	send_to_all_node((void *)hash_info.c_str()/*c_str()函数返回一个指向正规C字符串的指针常量, 内容与本string串相同*/, hash_info.size(), HASH_INFO_TAG);
...
void master::send_to_all_node(void * buf, int count, int tag) {////
	...
	//获得计算节点的数量
	const int COMPUTE_SIZE = m_machines.size();//m_machines是一个vector，存的工作节点号顺时针存的
...
void master::send_partition_info() {
	...
	std::string partition_info = m_ring->get_graph_info();//图信息，json形式的字符串
	std::stringstream ss;
	ptree pt;
	ss << partition_info;
	...
	for (int i = 0; i < COMPUTE_SIZE; i++) {
		...
		MPI_Isend((void *)(ss2.str().c_str()),//streamstring在调用str()时，会返回临时的string对象，c_str()函数返回一个指向正规C字符串的指针常量, 内容与本string串相同. 
			ss2.str().size(),
			MPI_BYTE,
			index_to_machine[i],
			GRAPH_INFO_TAG,
...
void master::distributing_graph() {
	...
	ecgraph::edge_t * read_buf;//申请数组
	ecgraph::edge_t ** send_buf;//申请二维数组
	try {
		read_buf = new ecgraph::edge_t[READ_GRAPH_DATA_ONCE];//0.1M
	}
	catch (std::bad_alloc) {
	...
	for (int i = 0; i < COMPUTE_SIZE; i++) {	//为所有的计算节点申请发送缓存空间
		try {
			send_buf[i] = new ecgraph::edge_t[GRAPH_DATA_BUFFER_SIZE];//0.5M
		}
		catch (std::bad_alloc){
	...
	while (!m_graphfile_buffer->is_over()) {
		...
		int readed_num = m_graphfile_buffer->read(read_buf, READ_GRAPH_DATA_ONCE);//0.1M
	...
	while (go_on) { //状态不为未完成，则继续
		switch (get_current_state()) {
			...
			case NODE_STATE::IN_ITERATION:
			{
				...
				m_machine_is_stop.clear();//map
			
				//LOG_TRIVIAL(info) << "master(" << m_rank << ") recving stop msg";
				recv_msg_from_worker();//waitall
...
void master::recv_msg_from_worker()
{
	...
	for (int i = 0; i < COMPUTE_SIZE; i++) {
		try {
			recvmsg_buf[i] = new ecgraph::byte_t[MAX_NONDATA_SIZE];//1k
		}
		catch (std::bad_alloc) {
...
void master::broadcast_msg_to_all(base_message * msg)//指向继承base_message这一类范围的对象
{
	std::string json_msg = msg->serialize();//
...
inline void master::handle_message(worker_runtime_info_msg &msg)
{
	...
	auto iter = m_run_info.find(worker_id);//map<vertex_t, worker_run_info_t>，find返回一个迭代器
