...
void graph_data_sort(engine<update_type> *algorithm, std::string partition_filename) {
	...
	while (!file_buf.is_over()) {
		...
		part.write((char *)edge_buf, read_num*sizeof(ecgraph::edge_t));//生成一个个排序好的临时文件
	}
	file_buf.write_join();
	...
	for (int i = index; i != 1; i = (i / 2 + i % 2)) {//归并排序
		...
		for (int j = 0; j < i; j += 2) {
			...
			if ((j + 1) == i) {
				//改名就行
				if (rename((sort_name + "_" + std::to_string(j)).c_str(),//成功执行时，返回0。失败返回-1
			...
			if (remove((sort_name + "_" + std::to_string(j)).c_str()) != 0) {//c_str()函数返回一个指向正规C字符串的指针常量,
