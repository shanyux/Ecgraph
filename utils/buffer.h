...
namespace ecgraph {
	...
	public:
		...
		buffer(unsigned int size):m_len(size),
                                    ...
                                    m_write_count(0),
                                    m_thrd(NULL){
			m_data = new elem_type[size]();//存放数据
			m_elem_size = sizeof(elem_type);
			buffer_mutex = new std::mutex();
		}
	...
	private:
		...
		int enqueue(elem_type *buf, int n){
			...
			if ( (m_rear + in_num ) <= m_len){//后面的空间很大
				memcpy((void * )(m_data + m_rear),
                        (void *)buf ,
					in_num * m_elem_size);
			}
			else{//前面有很大的空间
				int temp_num = m_len - m_rear;//后面还有多少空间
				memcpy((void *) (m_data + m_rear),
                        (void *)buf, 
                        temp_num * m_elem_size );
				memcpy((void *)m_data,//从开头存
                        (void *)(buf + temp_num), 
		...
		int dequeue(elem_type * buf, int n){
			...
			if ( (m_front + out_num) <= m_len){//在同一个圈上，m_front<m_rear  void *memcpy(void *dest, void *src, size_t count);
				memcpy((void *)buf,
                        (void *)(m_data + m_front),
                        out_num * m_elem_size);
			}
	...
	public:
		...
		int write( std::string filename ){//把数据写到buffer里面去
			reset_over();
		...
		void start_write(std::string filename){
            ...
            auto f = std::bind(&buffer::write, this, filename);//创建一个分支线程,一边读一边写
		...
		void write_join(){//写线程结束
			if (m_thrd){
				if (m_thrd->joinable() ){//如果是一个正在执行的线程，那么它是joinable的。
					m_thrd->join();//当该线程执行完成后才返回。（即等待子线程执行完毕才继续执行主线程）
				}
				delete m_thrd;
				m_thrd = NULL;
			}
		...
		bool is_over(){
			...
			ecgraph::scoped_lock lock(*buffer_mutex);
			return (m_over == 1) && (is_empty()) ;//？？？m_over
		}
		void set_over() { //使结束
			ecgraph::scoped_lock lock(*buffer_mutex);
			m_over = 1;
		...
		void reset_over() {
			ecgraph::scoped_lock lock(*buffer_mutex);//上锁
			m_over = 0;
		}
		void reset(){
			m_over = 0;
